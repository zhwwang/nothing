 ...
 * <p>
 * This class encapsulates functionalities to access to different parts of an
 * email address without dealing with its parsing.
 * </p>
 *
 * <p>
 * A MailAddress is an address specified in the MAIL FROM and RCPT TO commands
 * in SMTP sessions. These are either passed by an external server to the
 * mailet-compliant SMTP server, or they are created programmatically by the
 * mailet-compliant server to send to another (external) SMTP server. Mailets
 * and matchers use the MailAddress for the purpose of evaluating the sender and
 * recipient(s) of a message.
 * </p>
 *
 * <p>
 * MailAddress parses an email address as defined in RFC 821 (SMTP) p. 30 and 31
 * where addresses are defined in BNF convention. As the mailet API does not
 * support the aged "SMTP-relayed mail" addressing protocol, this leaves all
 * addresses to be a <mailbox>, as per the spec. The MailAddress's "user" is the
 * <local-part> of the <mailbox> and "host" is the <domain> of the mailbox.
 * </p>
 *
 * <p>
 * This class is a good way to validate email addresses as there are some valid
 * addresses which would fail with a simpler approach to parsing address. It
 * also removes parsing burden from mailets and matchers that might not realize
 * the flexibility of an SMTP address. For instance, "serge@home"@lokitech.com
 * is a valid SMTP address (the quoted text serge@home is the user and
 * lokitech.com is the host). This means all current parsing to date is
 * incorrect as we just find the first @ and use that to separate user from
 * host.
 * </p>
 *
 * <p>
 * This parses an address as per the BNF specification for <mailbox> from RFC
 * 821 on page 30 and 31, section 4.1.2. COMMAND SYNTAX.
...
public class MailAddress implements java.io.Serializable {
	 ...
	 * handle mails contaning RFC-821 source-route information
	 */
	 ...
	 *
	 * <p>
	 * The <code>personal</code> variable is left empty.
	 * </p>
	 *
	 * @param address
	...
	public MailAddress(String address) throws ParseException {
		...
		// Test if mail address has source routing information (RFC-821) and get
		// rid of it!!
		...
		try {
			...
			if (address.charAt(pos) == '\"') {
				...
				if (userSB.toString().length() == 2) {
					throw new ParseException(
							"No quoted local-part (user account) found at position "
									+ (pos + 2));
				}
			} else {
				...
				if (userSB.toString().length() == 0) {
					throw new ParseException(
							"No local-part (user account) found at position "
									+ (pos + 1));
				}
			}
			...
			if (pos >= address.length() || address.charAt(pos) != '@') {
				throw new ParseException(
						"Did not find @ between local-part and domain at position "
								+ (pos + 1));
			}
			pos++;
	 ...
	 *
	 * @param user
	 *            the username or account name on the mail server
	 * @param host
	 ...
	 *
	 * @return a <code>String</code> object representing the host part of this
	 *         email address. If the host is of the dotNum form (e.g.
	 *         [yyy.yyy.yyy.yyy]) then strip the braces first.
	 */
	 ...
	 *
	 * @return a <code>String</code> object representing the user part of this
	 *         email address.
	...
	public String toString() {
		StringBuffer addressBuffer = new StringBuffer(128).append(user)
				.append("@").append(host);
	...
	private String parseQuotedLocalPart(String address) throws ParseException {
		...
		while (true) {
			...
			if (address.charAt(pos) == '\\') {
				...
				if (x < 0 || x > 127) {
					throw new ParseException(
							"Invalid \\ syntaxed character at position "
									+ (pos + 1));
				}
				resultSB.append(x);
				pos++;
				...
				if (q <= 0 || q == '\n' || q == '\r' || q == '\"' || q == '\\') {
					throw new ParseException(
							"Unquoted local-part (user account) must be one of the 128 ASCI characters exception <CR>, <LF>, quote (\"), or backslash (\\) at position "
									+ (pos + 1));
				}
				resultSB.append(q);
				pos++;
	...
	private String parseUnquotedLocalPart(String address) throws ParseException {
		...
		while (true) {
			...
			if (address.charAt(pos) == '\\') {
				...
				if (c <= 31 || c >= 127 || c == ' ') {
					throw new ParseException(
							"Invalid character in local-part (user account) at position "
									+ (pos + 1));
				}
		...
		if (lastCharDot) {
			throw new ParseException(
					"local-part (user account) ended with a \".\", which is invalid.");
		}
		return resultSB.toString();
	}
	...
	private String parseNumber(String address) throws ParseException {
		...
		while (true) {
			...
			if (d < '0' || d > '9') {
				throw new ParseException(
						"In domain, did not find a number in # address at position "
								+ (pos + 1));
			}
			resultSB.append(d);
			pos++;
	...
	private String parseDotNum(String address) throws ParseException {
		...
		for (int octet = 0; octet < 4; octet++) {
			...
			if (address.charAt(pos) == ']') {
				if (octet < 3) {
					throw new ParseException(
							"End of number reached too quickly at " + (pos + 1));
				} else {
					break;
				}
			}
		...
		if (address.charAt(pos) != ']') {
			throw new ParseException(
					"Did not find closing bracket \"]\" in domain at position "
							+ (pos + 1));
		}
		resultSB.append(']');
		pos++;
	...
	private String parseDomainName(String address) throws ParseException {
		...
		if (result.startsWith("-") || result.endsWith("-")) {
			throw new ParseException(
					"Domain name cannot begin or end with a hyphen \"-\" at position "
							+ (pos + 1));
		}
		return result;
	}
